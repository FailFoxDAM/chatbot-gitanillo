...
async fn chat_handler(Json(payload): Json<ChatRequest>) -> Json<ChatResponse> {
    dotenv().ok();
    let api_key = env::var("GEMINI_API_KEY").unwrap_or_else(|_| "FALTA_KEY".to_string());
    let client = Client::new_from_model(Model::Gemini1_0Pro, api_key);
    ...
    match client.post(30, &txt_request).await {
        ...
        Err(e) => {
            // Mira tu terminal de Rust cuando le des a enviar en la web
            println!("❌ ERROR DE GEMINI: {:?}", e);
        }
    }
